...
def fetch_stock_data_cached(ticker, period="5y"):
    ...
    print(f"✅ {ticker} Data Fetched: {data.shape[0]} records BEFORE processing.")
    ...
    # ✅ Modified: Technical indicators with min_periods to retain more data
    data["SMA_20"] = data["Close"].rolling(20, min_periods=5).mean()
    data["EMA_20"] = data["Close"].ewm(span=20, adjust=False).mean()
    data["Momentum"] = data["Close"].diff(5).fillna(0)

    # ✅ Modified: RSI calculation with min_periods to prevent data loss
    ...
    print(f"✅ {ticker} Processed Data Available: {data.shape[0]} records AFTER processing.")
...
def preprocess_data(data):
    features = ["Close", "SMA_20", "EMA_20", "Momentum", "RSI"]
...
def train_or_load_model(X_train, y_train, seq_length=60, model_path="models/stock_model.keras"):
